...
async fn main() {
    ...
    let uri = env::var("MONGODB_URI")
        .expect("MONGODB_URI must be configured for persistent email storage");
    let client = Client::with_options(
        ClientOptions::parse(&uri)
            .await
            .expect("MONGODB_URI is invalid"),
    )
    .expect("MongoDB client could not be created");
    let db = client
        .default_database()
        .unwrap_or_else(|| client.database("email_manager"));
    let messages = db.collection::<EmailMessage>("messages");
    let suppressions = db.collection::<Suppression>("suppressions");
    let rate_counters = db.collection::<RateCounter>("rate_counters");
    ...
    messages
        ...
        .await
        .expect("email-manager id index could not be created");
    messages
        .create_index(
            IndexModel::builder()
        ...
        .await
        .expect("email-manager queue index could not be created");
    suppressions
        .create_index(
            IndexModel::builder()
                .keys(doc! { "email": 1 })
        ...
        .await
        .expect("email-manager suppression index could not be created");
    ...
    let state = AppState {
        ...
        mail_from_name: env::var("MAIL_FROM_NAME").unwrap_or_else(|_| "Eco".to_string()),
        per_recipient_day: env::var("EMAIL_PER_RECIPIENT_DAY")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(5),
        global_per_hour: env::var("EMAIL_GLOBAL_PER_HOUR")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(400),
        warmup_max_per_hour: env::var("EMAIL_WARMUP_MAX_PER_HOUR")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(400),
        warmup_days: env::var("EMAIL_WARMUP_DAYS")
            .ok()
            .and_then(|v| v.parse().ok())
            .unwrap_or(5),
    ...
    tokio::spawn(async move {
        loop {
            if let Err(error) = worker_tick(&worker_state).await {
...
async fn enqueue_message(state: &AppState, payload: SendRequest) -> Result<String, ApiError> {
    ...
    // Refuse to queue for a recipient we know we must never send to.
...
async fn unsubscribe(
    ...
    state
        ...
        .await
        .map_err(db_error)?;
    // Mark any queued/failed messages to this recipient as suppressed.
    state
        .messages
        .update_many(
...
async fn worker_tick(state: &AppState) -> anyhow::Result<()> {
    // Serialize queue draining across ticks (only one worker processes).
    ...
    // Global hourly budget: how many may we send this hour?
    ...
    // Warm-up ramp: new sender domains start slow and ramp up linearly.
    ...
    for message in messages {
        if let Err(error) = send_one(state, &message).await {            tracing::error!(id = %message.id, error = %error, "email-manager send failed");
            ...
            // Only count against budgets once we actually handed it to Brevo.
            state
                .rate_counters
                .update_one(
                    doc! { "key": &global_key },
...
/// Linear warm-up ramp: day 1 = max/5, day 5 = max. After warmup_days the
/// cap is `warmup_max_per_hour`.
fn warmup_rate(state: &AppState, now: DateTime<Utc>) -> u32 {
    if state.warmup_days == 0 || state.warmup_max_per_hour == 0 {
        return state.warmup_max_per_hour;
    }
    let day_index = now.ordinal() % state.warmup_days; // resets weekly-ish for stable daily stepping
    let max = state.warmup_max_per_hour;
    let step = max / state.warmup_days.max(1);
    (step * (day_index + 1)).min(max).max(1)
}

async fn send_one(state: &AppState, message: &EmailMessage) -> anyhow::Result<()> {
    ...
    if !status.is_success() {
        ...
        // Brevo returns 400 with a "code" when it rejects a recipient. If the
        // error looks permanent (invalid/bounced address), suppress it so we
        // don't retry forever.
...
impl AppState {
    fn public_unsubscribe_url(&self) -> String {
